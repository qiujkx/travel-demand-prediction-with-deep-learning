...
with tf.name_scope("Max_Pooling2") as scope:
  ...
  a_pool2_flatten = tf.reshape(a_pool2, [-1, int(a_pool2.get_shape()[1]) * filters_2])

with tf.name_scope("Fully_Connected1") as scope:
  W_fc1 = tf.Variable(tf.truncated_normal(shape=[int(a_pool2.get_shape()[1]) * filters_2, num_units], stddev=0.01))
  b_fc1 = tf.Variable(tf.constant(0.1, shape=[num_units]),name = 'b_fc1')
  pred = tf.matmul(a_pool2_flatten, W_fc1) + b_fc1
...
with tf.Session() as sess:
    ...
    for i in range(bootstrap_size, len(X), n_test):
      ...
      for epoch in range(num_epochs):
        for num_batch in range(len(X_train) / batch_size):
          ...
          _ = sess.run(opt, feed_dict={ input_placeholder: batch_X_train,
  ...
  errors = pd.DataFrame()
  errors['rmse'] = rmses
  errors.to_csv('data/cnn_errors.csv')
