...
class RNN_NeuralModel:
    ...
    def run_epochs(self, X, removed_seasonality, removed_std, location):
        """
        ...
        for i in range(bootstrap_size_train, len(X), n_test):
            ...
            for epoch in range(self.config.num_epochs):
                for num_batch in range(len(X_train) / self.config.batch_size):
                    ...
                    _, outH = sess.run([self.optimize, self.H],
            ...
            preds = sess.run(self.prediction,
                             feed_dict={self.input_placeholder: x_test,
        ...
        error = self.evaluate(preds_lstm, ground_truth)
        return error
...
def main():
    ...
    pd.DataFrame(stacked_errors).to_csv(
        'data/EWR_single_errors.csv', index=False, sep="\t")
